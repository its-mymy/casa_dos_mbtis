document.addEventListener("DOMContentLoaded",()=> {
    ...
    configurarCor("corFundo","corFundoHex",cor=>card.style.background=cor);

    configurarCor("corCelular","corCelularHex",cor=>{
        phone.style.background=`linear-gradient(145deg,rgba(255,255,255,.5) 0%,rgba(255,255,255,.08) 25%,transparent 45%),${cor}`;
    });

    configurarCor("corBotoes","corBotoesHex",cor=>{
        document.querySelectorAll(".info-button").forEach(btn=>btn.style.background=cor);
    });

    configurarCor("corBrilhos","corBrilhosHex",cor=>{
        card.style.setProperty("--sparkle-color",cor);
        document.querySelectorAll(".sparkle,.phone-star").forEach(el=>el.style.color=cor);
    });
    ...
    const posicoes=[
        "accessory-position-1","accessory-position-2","accessory-position-3","accessory-position-4",
        "accessory-position-5","accessory-position-6","accessory-position-7","accessory-position-8"
    ];
    ...
    function ajustarPreview(){
        const larguraDisponivel=Math.min(wrapper.clientWidth,window.innerWidth-20);
        const escala=Math.min(.78,Math.max(.25,(larguraDisponivel-10)/800));
